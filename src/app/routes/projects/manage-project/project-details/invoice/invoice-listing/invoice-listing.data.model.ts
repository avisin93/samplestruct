...
export class ManageInvoiceListData {
    /**
            ...
            for (let i = 0; i < poList.length; i++) {
                ...
                const invoiceObj = {
                    ...
                    'approvalList': (dataObj.approvalList && dataObj.approvalList.length > 0) ?
                        ManageInvoiceListData.getApprovalHierarchyData(dataObj.approvalList) : [],
        ...
        if (paymentsListIncommingData) {
            for (let i = 0; i < paymentsListIncommingData.length; i++) {
                ...
                const paymentsData = {
                    ...
                    'scheduleDate': data.scheduleDate ? ManageInvoiceListData.getDisplayDate(data.scheduleDate) : '',
