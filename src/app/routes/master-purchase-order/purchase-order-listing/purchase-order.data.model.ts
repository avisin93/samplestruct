...
import { DatePickerMethods } from '@app/common';
import { PAYMENT_APPROVAL_CONSTANTS } from '../../projects/manage-project/project-details/purchase-order/purchase-order.constants';
import { PO_APPROVAL_CONST } from '@app/config/constants';


export class MasterPOListData {
  /**
    ...
    if (poList && poList.length > 0) {
      for (let i = 0; i < poList.length; i++) {
        ...
        const poObj = {
          ...
          'status': dataObj.status === PO_APPROVAL_CONST.generated ? PO_APPROVAL_CONST.pending : dataObj.status,
          ...
          'approvalList': (dataObj.approvalList && dataObj.approvalList.length > 0) ? MasterPOListData.getApprovalHierarchyData(dataObj.approvalList) : [],
          'paymentsApprovalList': (dataObj.paymentsApprovalList && dataObj.paymentsApprovalList.length > 0) ? MasterPOListData.getApprovalHierarchyData(dataObj.paymentsApprovalList) : [],
